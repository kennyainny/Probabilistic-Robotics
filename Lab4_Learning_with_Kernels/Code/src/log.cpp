...
void pick_two_classes(log_type log, log_type *log_two){
  long node_1[log.count] = {0}, node_2[log.count] = {0};
  long count = 0, k;
  int label1 = LABEL1, label2 = LABEL2;
  for(int i = 0; i < log.count; i++){
    if(log.node_label[i] == label1){
      count = count + 1;
      node_1[i] = log.node_id[i] + 1;
    }else if(log.node_label[i] == label2){
      count = count + 1;
      node_2[i] = log.node_id[i] + 1;
    }
  }
  cout << count << endl;
  log_two->count = count;
  new_hornetsoft_log(log_two);
  count = 0;
  for(int i = 0; i < log.count; i++){
    if(log.node_label[i] == label1){
      k = node_1[i] - 1;
      log_two->node_id[count] = count;
      log_two->node_label[count] = label1;
      log_two->point[count] = log.point[k];
      log_two->feature[count] = log.feature[k];
      count = count + 1;
    }else if(log.node_label[i] == label2){
      k = node_2[i] - 1;
      log_two->node_id[count] = count;
      log_two->node_label[count] = label2;
      log_two->point[count] = log.point[k];
      log_two->feature[count] = log.feature[k];
      count = count + 1;
    }
